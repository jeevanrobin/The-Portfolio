...
function ExpCard({ exp, index }) {
  ...
  return (
    <div ref={ref} className={`reveal ${revealed?"is-revealed":""}`}
      style={{ transitionDelay:`${index*0.12}s`, background:"rgba(255,255,255,0.03)", border:"1px solid rgba(255,255,255,0.08)", borderRadius:"1rem", padding:"2rem", transition:"border-color 0.3s" }}
      ...
      <div style={{ display:"flex", flexWrap:"wrap", justifyContent:"space-between", gap:"0.75rem", marginBottom:"1.25rem" }}>
        <div>
          <div style={{ display:"flex", alignItems:"center", gap:"0.5rem", marginBottom:"0.25rem" }}>
            ...
            {exp.current && <span style={{ display:"inline-flex", alignItems:"center", gap:"0.3rem", fontSize:"0.6rem", textTransform:"uppercase", color:"#89AACC", background:"rgba(137,170,204,0.1)", borderRadius:"9999px", padding:"0.2rem 0.5rem" }}>
      ...
      <ul style={{ listStyle:"none", padding:0, margin:"0 0 1.25rem", display:"flex", flexDirection:"column", gap:"0.5rem" }}>
        {exp.points.map(pt => (
          <li key={pt} style={{ display:"flex", gap:"0.5rem", fontSize:"0.85rem", color:"#878787" }}>
            <span style={{ marginTop:"0.45rem", width:"4px", height:"4px", borderRadius:"50%", background:"#89AACC", flexShrink:0 }} />{pt}
          </li>
        ))}
      </ul>
      <div style={{ display:"flex", flexWrap:"wrap", gap:"0.375rem" }}>
        {exp.tags.map(tag => <span key={tag} style={{ fontSize:"0.7rem", color:"#878787", background:"rgba(0,0,0,0.4)", border:"1px solid rgba(255,255,255,0.07)", borderRadius:"9999px", padding:"0.2rem 0.6rem" }}>{tag}</span>)}
      </div>
    </div>
  );
}
    ...
    <section id="experience" style={{ background:"#0a0a0a", padding:"6rem 1.5rem", borderTop:"1px solid rgba(255,255,255,0.05)" }}>
      <div style={{ maxWidth:"56rem", margin:"0 auto" }}>
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:"1rem" }}>
          {exps.map((exp, i) => <ExpCard key={exp.company} exp={exp} index={i} />)}
        </div>
      </div>
